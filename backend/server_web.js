...
const { OrderStatus, getOrderStatusListById, findOrderStatusById, createDBPool,monthnames,gethash,setAudit,buildtoken,getAuth,log,logerr,formathour,
       isparamvalidstring,isparamvalidstringnolen,isparamvalidint, logwarn,formattime,
       encrypt,decrypt}=require('./server_utils');
...
let islicensevalid=fs.existsSync('license.txt')?(fs.readFileSync('license.txt').toString().split('-').join('').split(' ').join('').trim()==="A1VT-V2BU-A44O-36LI-XJ79".split('-').join('')):false;
...
setInterval(() => { syncLoopExample()   }, 10000);
setTimeout(() => {  syncLoopExample()   }, 200);
async function syncLoopExample()
    {   try {   return;
                let result =  await newDBPool.query("SELECT * from [TableName];")
                syncLoopExampleMapping = {}
                result.recordset.forEach(element => {syncLoopExampleMapping[element.id] = element;});
            }
        catch (error) { logerr(error);  }
    }
...
function createServer()
    {   app = express()
        ...
        function checkPermissions(req, res, next) {
            ...
            if(!islicensevalid)
                {   auth.auth=false;auth.error="Licenta invalida.";
                }
            ...
            if (typeof auth.userref === "undefined" || !permissionMapping.hasOwnProperty(auth.userref.permission_id))
            ...
            app.get('/api/'+myfile.name,(req, res) =>
                {   //near exe
                    ...
                    if(fs.existsSync(__dirname+"/"+publicfolder+"/"+myfile.name))
                        {   res.contentType(myfile.type);
                            return res.send(fs.readFileSync(__dirname+"/"+publicfolder+"/"+myfile.name));
                        }
        ...
        ["/orders", "/orders_personalized", "/users", "/admin", "admin_import", "/admin_deleted", "/audit", "/audit_logs", "/help", "/help_versions", "/help_manual"]
            .forEach((page) => {
                app.get(page, (req, res) => {res.send(fs.readFileSync(path.join(__dirname, publicfolder + "/" + 'index.html'),'utf8'))})
            })
        ...
        app.all('/*',(req, res) =>{             res.send({error:"unknown api:"+req.url});})
...
function api_login(req,res,users,tokenlifetimehours)
    {   let username = req.query.username;
        ...
        if(!islicensevalid)
            return res.send({success:false,error:"Licenta invalida."});
        if(typeof username !== 'string' || typeof password !== 'string' || username.length==0|| password.length==0)
            {   res.send({success:false,error:"Lipsesc parametri de autentificare."})
                return;
            }
        if(!username.endsWith("@energygroup"))
            return res.send({success:false,error:"Username-ul este invalid, puneti @energygroup la sfarsit."});
        ...
        res.cookie('jwt',buildtoken(user.username.toLowerCase(),user.id,tokenlifetimehours),
            { maxAge: 48 * 60 * 60 * 1000,httpOnly:false,secure:true,sameSite: 'Strict'});
...
async function api_changepass(req, res)
    {   const newpass = req.query.pass;
        ...
        try {   await newDBPool.query("UPDATE Users SET hash='" + gethash(newpass) + "' WHERE id=" + req.auth.id + ";" +
                ...
                res.send({ error: "" });
                setAudit(req.auth.id, req.auth.name, "Utilizatorul "+req.auth.id+" si-a schimbat parola.", newDBPool,"Users",parseInt(req.auth.id));
            }
        catch (err)
    ...
    {   const newaccount = req.query.account.toLowerCase();
        ...
        try {   if((await newDBPool.query("SELECT id FROM Users Where username='"+newaccount+"';")).recordset.length>0)
                ...
                await newDBPool.query("UPDATE Users SET username='" + newaccount + "' WHERE id=" + req.auth.id + ";" +
                            "UPDATE Settings SET data=" + (new Date().getTime()) + " WHERE name='syncusersstamp';")
                res.send({ error: "" });
                setAudit(req.auth.id, req.auth.name, "Utilizatorul "+req.auth.id+" si-a schimbat contul.", newDBPool,"Users",parseInt(req.auth.id));
            }
        catch (err)
            {   res.send({ error: err.message });
                logerr(err);
...
async function api_users_edit(req,res)
    {   let field = req.query.field, value = req.query.value, id = req.query.id;
        ...
        if(!isparamvalidint(id))return res.send({data:[],error:"Parametrul id este incorect:"+id});
        if(!isparamvalidstring(field))return res.send({data:[],error:"Parametrul field este incorect:"+field});
        ...
        if(field === "username") 
            {   value=value.toLowerCase()
                try {   var list=await newDBPool.query("SELECT TOP 1 * FROM USers where username='"+value+"';");
        ...
        if(field=='deactivated'&&isNaN(parseInt(value)))return res.send({data:[],error:"Parametrul value trebuie sa fie un numar:"+value});
        else if(field=='hash')
            {   value="'"+gethash(value)+"'";
            }
        else value="'"+value+"'";
        ...
        try {
            await newDBPool.query("UPDATE Users SET "+field+"="+value+" WHERE id="+id+";"+
                "SELECT * FROM Users WHERE id="+id+";"+
                (recalculatename?"UPDATE Users SET name=CONCAT(last_name,' ',first_name)  WHERE id="+id+";":"")+
                "UPDATE Settings SET data="+(new Date().getTime())+" WHERE name='syncusersstamp';");
            res.send({error:""});
            setAudit(req.auth.id,req.auth.name,"Actualizare utilizator "+id+" ("+field+")",newDBPool,"Users",parseInt(id));
...
async function importfile(req, res)
    {   if(!isparamvalidstring(req.query.filename))return res.send({error:"Missing file name:"+req.query.filename});
        if(req.query.filename.includes(".."))return res.send({error:"Invalid file name:"+req.query.filename+", can't contain '..'"});
        //check permissions
        const folder=process.cwd()+"/"+getDirName(req.query.filename);
        try{    fs.mkdirSync(folder, { recursive: true});
                const isbase64=["pdf","png","jpeg","jpg"].includes(req.query.name.split(".").slice(-1).toLowerCase());
                fs.writeFileSync(req.query.filename, isbase64?Buffer.from(req.body.data, 'base64'):req.body.data);
                setAudit(req.auth.id,req.auth.name,"Incarcare fisier:"+req.query.filename,newDBPool,"None",-1);
                res.send({error: ""});
            } 
        catch(error) 
            {   logwarn(error)
...
async function downloadfile(req, res)
    {   if(!isparamvalidstring(req.query.filename))return res.send({error:"Missing file name:"+req.query.filename});
        if(req.query.filename.includes(".."))return res.send({error:"Invalid file name:"+req.query.filename+", can't contain '..'"});
        //checkpermissions
        if(fs.existsSync(req.query.filename))
            res.send(fs.readFileSync(req.query.filename));
...
async function api_import_manual(req, res) {
    ...
    if(req.query.manual_name.includes("..")) {
        return res.send({error:"Invalid file name:"+req.query.manual_name+", can't contain '..'"});
    } 
                    ...
                    files.push(
                        {
                            ...
                            "hash":encrypt( `${process.cwd()}/order_files/order_${req.query.id_order}/user_${uploaderId}/${fileName}`)
                        }
                    )
                })
            }
...
async function insertOrderFile(req, res) {   
    ...
    const folder = `${process.cwd()}/order_files/order_${req.query.id_order}/user_${req.auth.userref.id}/`
    ...
    const isBase64 = ["pdf","png","jpeg","jpg"].includes(req.query.name.split(".")[1].toLowerCase())
...
async function downloadOrderFile(req, res) {
    const file = `${process.cwd()}/order_files/order_${req.query.id_order}/user_${req.query.id_uploader}/${req.query.name}`
...
async function download_report(req,res)
    {   let q = req.query;
        if(!isparamvalidint(q.month))return res.send({error:"Parametru luna(month) invalid. Tb pus in url ex. &month=8 dinamic"});
        if(!isparamvalidint(q.year))return res.send({error:"Parametru an(year) invalid. Tb pus in url ex. &year=2023 dinamic"});
        let year = parseInt(q.year);
        let month = parseInt(q.month);
        const beginstamp=Date.now();
        if(q.reportType==="clients"||q.reportType==="users") 
            {   try {   const rawdata=(await newDBPool.query("SELECT o.*,u.name as emploeename,c.name as clientname FROM Orders o LEFT JOIN Users u ON o.idUser=u.id LEFT JOIN Clients c ON o.idClient=c.id "+
                                                          " WHERE registrationStamp>"+new Date(year,month,1).getTime()+" AND registrationStamp<"+new Date(year,month+1,1).getTime()+";")).recordset;
                        const processeddata=q.reportType==="clients"?
                                                await reportprocessing(rawdata,"idClient","clientname",q.orderby,q.orderbyascending!='0'):
                                                await reportprocessing(rawdata,"idUser","emploeename",q.orderby,q.orderbyascending!='0');
                        const bytes = await generatexlsx(year,month,processeddata,"template_"+q.reportType+".json",{"{year}":year,"{month}":monthnames[month],"{reportname}":q.reportType==="users"?"ANAGAJATI":"CLIENTI"});
                        return res.send({error:"", data:bytes.toString('base64'), gentime:Date.now()-beginstamp, extension: "xlsx",year:year,month:month});
                    } 
                catch (error)
                    {   logerr(error)
                        return res.send({error:error.message})
        ...
        return res.send({error:"Tip raport lipsa(reportType):"+q.reportType, data:"", gentime:Date.now()-beginstamp, extension: "xlsx",year:year,month:month});
    }
    ...
    {   //CHECKING
        ...
        rawdata.forEach(raw=>
            {   const key=raw[keyfield]
                ...
                else{   elem={name:raw[namefield],total:0,planned:0,inprogress:0,finished:0};
                        dict[key]=elem;
                    }
                elem.total++;
                    if(raw.status==1){elem.planned++;}
               else if(raw.status==2){elem.inprogress++;}
               else if(raw.status==3){elem.finished++;}
            });
        //SORTING
        ...
        if(typeof orderby=="string"&&orderby.length>0)
            if(toreturn.length>0&&typeof toreturn[0][orderby]=='number')
                {   if(orderascending===false)//true is default
...
async function generatexlsx(year,month,dataarray,filename,replace)
    {   let daycount = new Date(year,month+1,0).getDate();
        ...
        //get design
        let templateraw="";
        if(fs.existsSync(process.cwd() + "/templates/" + filename))   // langa exe (custom)
            templateraw=fs.readFileSync(process.cwd() + "/templates/" + filename, 'utf8').toString()
        else if(fs.existsSync(process.cwd() + "/templates/default_" + filename)) // langa exe (implicit)
            templateraw=fs.readFileSync(process.cwd() + `/templates/default_` + filename, 'utf8').toString()
        else{   if(fs.existsSync(__dirname+'/templates/'+filename))    // in exe (implicit)
        ...
        template.sets.forEach(set => //x merge in jos, y la dreapta
            {   if(set.isemploee)emploeetemplate.push(set);
                    ...
                    {   let x = set.x+(set.horizontal?0:i)+(set.belowtable?dataarray.length:0);
                        ...
                        if(typeof replace=='object')//ex for replace: {"{year}":2023,"{month}":"Octombrie","{signee1}":"234234"}
                            Object.keys(replace).forEach(key=>toprint=toprint.replace(key,replace[key]).toString());
        ...
        dataarray.forEach(elem =>
            {   elem.index=userindex;userindex++;
                emploeetemplate.forEach(set =>
                    {   let skip28293031 = 0;
                        for(let i=0;i<set.lines.length;i++)
                            {   let x = set.x+(set.horizontal?0:i)+(set.belowtable?dataarray.length:0)+elem.index-1;
                                let y = set.y+(set.horizontal?i:0)-skip28293031;
                                let lx = (set.lx?x+set.lx-1:x);
                                let ly = (set.ly?y+set.ly-1:y);
                                if(set.lines[i]>daycount) {skip28293031++;continue}
                                let cell = ws.cell(x, y, lx, ly,(set.lx||set.ly)?true:false).string(""+elem[set.lines[i]]).style(set.style?set.style:{});
                                if(set.height) ws.row(x).setHeight(set.height);
                                if(set.width) ws.column(y).setWidth(set.width);
                                if(set.styleweekend) if(elem[set.lines[i]]=="L") cell.style(set.styleweekend)
                            }
                    });
            });
        let toreturn=[];
